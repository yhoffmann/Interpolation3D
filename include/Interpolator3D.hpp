...
class Interpolator3D {
 ...
 public:
  ...
  struct DataGenerationConfig {
    ...
    double x_min = 0.0;
    double x_max = 15.0;
    GridSpacing x_grid_spacing = Exponential;
    double x_exp_grid_spacing_parameter = 8.0;
    uint ny = 400;
    double y_min = 0.0;
    double y_max = 15.0;
    GridSpacing y_grid_spacing = Exponential;
    double y_exp_grid_spacing_parameter = 8.0;
    uint nz = 60;
    double z_min = 0.0;
    double z_max = M_PI;
 ...
 protected:
  struct Coeffs {
   ...
   public:
    inline double& operator[](uint i) { return m_coeffs[i]; }
  };
 ...
 public:
  ...
  void generate_data(std::function<double(double, double, double)> func,
                     const DataGenerationConfig* config,
  ...
  static double bicubic_interpolate(double p[4][4], double t_y[2],
                                    double t_z[2], double y, double z);
 ...
 protected:
  ...
  static double A(Coeffs& coeffs, uint i, uint j, double z, double z2,
                  double z3);
  static double B(Coeffs& coeffs, uint i, double y, double y2, double y3,
                  double z, double z2, double z3);
  double get_pos_of_grid_point(Dir dir, int index,
                               const DataGenerationConfig* config) const;
  void find_closest_lower_data_point(int& i_0, int& j_0, int& k_0, double& x,
...
inline double Interpolator3D::get_pos_of_grid_point(
    Dir dir, int i, const DataGenerationConfig* config) const {
...
inline void Interpolator3D::safe_delete_cached_coeffs() {
  if (m_cached_coeffs != nullptr) delete[] m_cached_coeffs;

  m_cached_coeffs = nullptr;
}
  ...
  for (uint i = 0; i < m_nx; i++) {
    pool.enq_job([i, this] {
      for (uint j = 0; j < m_ny; j++)
        for (uint k = 0; k < m_nz; k++) set_single_cell_coeffs(i, j, k);
    });
  }
...
inline void Interpolator3D::set_single_cell_coeffs(uint i_0, uint j_0,
                                                   uint k_0) {
  double p[4][4][4];
  ...
  b[32] = (p[2][2][1] - p[2][0][1] - (p[0][2][1] - p[0][0][1])) * one_div_dx2 *
          one_div_dy2;
  b[33] = (p[3][2][1] - p[3][0][1] - (p[1][2][1] - p[1][0][1])) * one_div_dx3 *
          one_div_dy2;
  b[34] = (p[2][3][1] - p[2][1][1] - (p[0][3][1] - p[0][1][1])) * one_div_dx2 *
          one_div_dy3;
  b[35] = (p[3][3][1] - p[3][1][1] - (p[1][3][1] - p[1][1][1])) * one_div_dx3 *
          one_div_dy3;
  b[36] = (p[2][2][2] - p[2][0][2] - (p[0][2][2] - p[0][0][2])) * one_div_dx2 *
          one_div_dy2;
  b[37] = (p[3][2][2] - p[3][0][2] - (p[1][2][2] - p[1][0][2])) * one_div_dx3 *
          one_div_dy2;
  b[38] = (p[2][3][2] - p[2][1][2] - (p[0][3][2] - p[0][1][2])) * one_div_dx2 *
          one_div_dy3;
  b[39] = (p[3][3][2] - p[3][1][2] - (p[1][3][2] - p[1][1][2])) * one_div_dx3 *
          one_div_dy3;

  // partial xz derivatives
  b[40] = (p[2][1][2] - p[2][1][0] - (p[0][1][2] - p[0][1][0])) * one_div_dx2 *
          one_div_dz2;
  b[41] = (p[3][1][2] - p[3][1][0] - (p[1][1][2] - p[1][1][0])) * one_div_dx3 *
          one_div_dz2;
  b[42] = (p[2][2][2] - p[2][2][0] - (p[0][2][2] - p[0][2][0])) * one_div_dx2 *
          one_div_dz2;
  b[43] = (p[3][2][2] - p[3][2][0] - (p[1][2][2] - p[1][2][0])) * one_div_dx3 *
          one_div_dz2;
  b[44] = (p[2][1][3] - p[2][1][1] - (p[0][1][3] - p[0][1][1])) * one_div_dx2 *
          one_div_dz3;
  b[45] = (p[3][1][3] - p[3][1][1] - (p[1][1][3] - p[1][1][1])) * one_div_dx3 *
          one_div_dz3;
  b[46] = (p[2][2][3] - p[2][2][1] - (p[0][2][3] - p[0][2][1])) * one_div_dx2 *
          one_div_dz3;
  b[47] = (p[3][2][3] - p[3][2][1] - (p[1][2][3] - p[1][2][1])) * one_div_dx3 *
          one_div_dz3;

  // partial yz derivatives
  b[48] = (p[1][2][2] - p[1][2][0] - (p[1][0][2] - p[1][0][0])) * one_div_dy2 *
          one_div_dz2;
  b[49] = (p[2][2][2] - p[2][2][0] - (p[2][0][2] - p[2][0][0])) * one_div_dy2 *
          one_div_dz2;
  b[50] = (p[1][3][2] - p[1][3][0] - (p[1][1][2] - p[1][1][0])) * one_div_dy3 *
          one_div_dz2;
  b[51] = (p[2][3][2] - p[2][3][0] - (p[2][1][2] - p[2][1][0])) * one_div_dy3 *
          one_div_dz2;
  b[52] = (p[1][2][3] - p[1][2][1] - (p[1][0][3] - p[1][0][1])) * one_div_dy2 *
          one_div_dz3;
  b[53] = (p[2][2][3] - p[2][2][1] - (p[2][0][3] - p[2][0][1])) * one_div_dy2 *
          one_div_dz3;
  b[54] = (p[1][3][3] - p[1][3][1] - (p[1][1][3] - p[1][1][1])) * one_div_dy3 *
          one_div_dz3;
  b[55] = (p[2][3][3] - p[2][3][1] - (p[2][1][3] - p[2][1][1])) * one_div_dy3 *
          one_div_dz3;

  // partial xyz derivatives
  b[56] = (p[2][2][2] - p[2][2][0] - (p[2][0][2] - p[2][0][0]) -
           (p[0][2][2] - p[0][2][0]) + (p[0][0][2] - p[0][0][0])) *
          one_div_dx2 * one_div_dy2 * one_div_dz2;
  b[57] = (p[3][2][2] - p[3][2][0] - (p[3][0][2] - p[3][0][0]) -
           (p[1][2][2] - p[1][2][0]) + (p[1][0][2] - p[1][0][0])) *
          one_div_dx3 * one_div_dy2 * one_div_dz2;
  b[58] = (p[2][3][2] - p[2][3][0] - (p[2][1][2] - p[2][1][0]) -
           (p[0][3][2] - p[0][3][0]) + (p[0][1][2] - p[0][1][0])) *
          one_div_dx2 * one_div_dy3 * one_div_dz2;
  b[59] = (p[3][3][2] - p[3][3][0] - (p[3][1][2] - p[3][1][0]) -
           (p[1][3][2] - p[1][3][0]) + (p[1][1][2] - p[1][1][0])) *
          one_div_dx3 * one_div_dy3 * one_div_dz2;
  b[60] = (p[2][2][3] - p[2][2][1] - (p[2][0][3] - p[2][0][1]) -
           (p[0][2][3] - p[0][2][1]) + (p[0][0][3] - p[0][0][1])) *
          one_div_dx2 * one_div_dy2 * one_div_dz3;
  b[61] = (p[3][2][3] - p[3][2][1] - (p[3][0][3] - p[3][0][1]) -
           (p[1][2][3] - p[1][2][1]) + (p[1][0][3] - p[1][0][1])) *
          one_div_dx3 * one_div_dy2 * one_div_dz3;
  b[62] = (p[2][3][3] - p[2][3][1] - (p[2][1][3] - p[2][1][1]) -
           (p[0][3][3] - p[0][3][1]) + (p[0][1][3] - p[0][1][1])) *
          one_div_dx2 * one_div_dy3 * one_div_dz3;
  b[63] = (p[3][3][3] - p[3][3][1] - (p[3][1][3] - p[3][1][1]) -
           (p[1][3][3] - p[1][3][1]) + (p[1][1][3] - p[1][1][1])) *
          one_div_dx3 * one_div_dy3 * one_div_dz3;

  for (uint i = 0; i < 64; i++) {
    double element = 0.0;
    for (uint j = 0; j < 64; j++) element += COEFF_MATRIX[i][j] * b[j];
...
inline void Interpolator3D::set_grid(const DataGenerationConfig* config) {
  ...
  m_x = new (
      std::nothrow) double[m_nx + 3];  // +3 because we need space for one more
                                       // to the left and two more to the right
  ...
  if (config) {
    for (uint i = 0; i < m_nx; i++)
      m_x[i + 1] = get_pos_of_grid_point(
          Dir::X, i,
          config);  // +1 because we do not fill the two outermost elements yet
  ...
  for (uint i = 0; i < m_nx + 3; i++)
    for (uint j = 0; j < m_ny + 3; j++)
      for (uint k = 0; k < m_nz + 3; k++) {
        ...
        if (is_outermost)
          m_data[get_data_index(i, j, k)] =
              m_data[get_data_index(i_temp, j_temp, k_temp)];
      }
}

inline void Interpolator3D::export_data_old_format(
    const std::string& filepath) const {
#ifdef _INTERP_LOG
  // checking if file already exists
  std::ifstream file_check(filepath);
  if (file_check) {
    std::cout << "This file already exists and would be overwritten if data "
                 "was exported. Do you want to overwrite the file? (y/n)"
              << std::endl;
    std::string answer;
    std::cin >> answer;
    if (answer != "y") {
  ...
  for (uint i = 0; i < m_nx; i++) {
    for (uint j = 0; j < m_ny; j++) {
      for (uint k = 0; k < m_nz; k++) {
        out << std::setprecision(10) << i << " " << j << " " << k << " "
            << m_x[i + 1] << " " << m_y[j + 1] << " " << m_z[k + 1] << " "
            << m_data[get_data_index(i + 1, j + 1, k + 1)] << std::endl;
      }
      out << std::endl;
    }
    out << std::endl;
...
inline void Interpolator3D::import_data_old_format(
  ...
  std::string element;
  while (std::getline(ss, element, ' ')) line_vec.push_back(element);

  if (line_vec[0] != "#N") {
#ifdef _INTERP_LOG
    std::cerr << "Wrong m_data format, use generate_data() to generate m_data "
  ...
  set_grid(nullptr);  // allocate memory for position arrays but leave the
                      // position arrays uninitialized
  ...
  while (std::getline(in, line)) {
    if (line != "") {
      ...
      m_x[i + 1] = std::stod(
          line_vec[3]);  // these assignments are done unnecessarily often but I
                         // cant be bothered to do this smarter
...
inline void Interpolator3D::export_data_plain_text(
    const std::string& filepath) const {
#ifdef _INTERP_LOG
  std::ifstream file_check(filepath);
  if (file_check) {
  ...
  }  // TODO fix potential error that might occur when the file gets changed
     // (for example renamed) while already opened by this function
  file_check.close();
  ...
  out << "#n " << m_nx << " " << m_ny << " " << m_nz
      << std::endl;  // printing m_nx, m_ny, m_nz into the first line
  ...
  for (uint i = 1; i < m_nx + 1; i++)
    for (uint j = 1; j < m_ny + 1; j++)
      for (uint k = 1; k < m_nz + 1; k++)
        out << std::setprecision(10) << m_data[get_data_index(i, j, k)]
            << std::endl;
...
inline void Interpolator3D::import_data_plain_text(
    const std::string& filepath) {
#ifdef _INTERP_LOG
  ...
  if (line_vec[0] == "#N") {
    ...
    std::cerr
        << "=========="
           "\n"
           "You are using the new function to import data that is in the old "
           "format. This will work but you should switch to new data format by "
           "just exporting data again with export_data. Old format will remain "
           "available for export with export_data_old_format."
  ...
  uint i(0), j(0), k(0);
  while (std::getline(in, line) && line != "") m_x[++i] = std::stod(line);

  while (std::getline(in, line) && line != "") m_y[++j] = std::stod(line);

  while (std::getline(in, line) && line != "") m_z[++k] = std::stod(line);
...
inline void Interpolator3D::export_data(const std::string& filepath) const {
  ...
  out.write(reinterpret_cast<const char*>(m_data),
            sizeof(double) * (m_nx + 3) * (m_ny + 3) * (m_nz + 3));
...
inline void Interpolator3D::import_data(const std::string& filepath) {
  ...
  // data
  prepare_data();
  in.read(reinterpret_cast<char*>(m_data),
          sizeof(double) * (m_nx + 3) * (m_ny + 3) * (m_nz + 3));
...
inline void Interpolator3D::generate_data(
    std::function<double(double, double, double)> func,
    const DataGenerationConfig* config, bool monitor_progress) {
  set_grid(config);
  prepare_data();
...
inline void Interpolator3D::find_closest_lower_data_point(int& i_0, int& j_0,
                                                          int& k_0, double& x,
                                                          double& y,
                                                          double& z) const {
...
inline double Interpolator3D::unicubic_interpolate(double p[4], double t[2],
                                                   double z) {
  double p2_m_p0_div_t2_m_t0 = (p[2] - p[0]) / (1.0 - t[0]);
  double p3_m_p1_div_t3_m_t1 = (p[3] - p[1]) / t[1];
  double p1_m_p2 = p[1] - p[2];

  return p[1] + z * p2_m_p0_div_t2_m_t0 +
         z * z *
             (-3.0 * p1_m_p2 - 2.0 * p2_m_p0_div_t2_m_t0 -
              p3_m_p1_div_t3_m_t1) +
         z * z * z *
             (2.0 * p1_m_p2 + p2_m_p0_div_t2_m_t0 + p3_m_p1_div_t3_m_t1);
}

inline double Interpolator3D::bicubic_interpolate(double p[4][4], double t_y[2],
                                                  double t_z[2], double y,
                                                  double z) {
...
inline double Interpolator3D::tricubic_interpolate(double p[4][4][4],
                                                   double t_x[2], double t_y[2],
                                                   double t_z[2], double x,
                                                   double y, double z) {
...
inline double Interpolator3D::get_interp_value_bicubic_unilinear(
  ...
  double t_x[4];
  double t_y[4];
  for (int i = 0; i < 4; i++) t_x[i] = m_x[i + i_0];
  for (int i = 0; i < 4; i++) t_y[i] = m_y[i + j_0];

  double one_div_t_x_2_t_x_1 =
      1.0 / (t_x[2] -
             t_x[1]);  // this looks like this because of performance advantages
...
inline double Interpolator3D::get_interp_value_tricubic_old(double x, double y,
                                                            double z) const {
  int i_0, j_0, k_0;
  ...
  double t_x[2];
  double t[4];
  for (int i = 0; i < 4; i++) t[i] = m_x[i + i_0];
  ...
  double t_y[2];
  for (int i = 0; i < 4; i++) t[i] = m_y[i + j_0];
  ...
  double t_z[2];
  for (int i = 0; i < 4; i++) t[i] = m_z[i + k_0];
  one_div_t_2_m_t_1 = 1.0 / (t[2] - t[1]);
...
inline double Interpolator3D::A(Coeffs& coeffs, uint i, uint j, double z,
                                double z2, double z3) {
  return coeffs[i + j * 4 + 0 * 16] + coeffs[i + j * 4 + 1 * 16] * z +
         coeffs[i + j * 4 + 2 * 16] * z2 + coeffs[i + j * 4 + 3 * 16] * z3;
}

inline double Interpolator3D::B(Coeffs& coeffs, uint i, double y, double y2,
...
inline double Interpolator3D::get_interp_value_tricubic(double x, double y,
  ...
  return B(m_cached_coeffs[i_0 * m_ny * m_nz + j_0 * m_nz + k_0], 0, y, y2, y3,
           z, z2, z3) +
         B(m_cached_coeffs[i_0 * m_ny * m_nz + j_0 * m_nz + k_0], 1, y, y2, y3,
           z, z2, z3) *
             x +
         B(m_cached_coeffs[i_0 * m_ny * m_nz + j_0 * m_nz + k_0], 2, y, y2, y3,
           z, z2, z3) *
             x2 +
         B(m_cached_coeffs[i_0 * m_ny * m_nz + j_0 * m_nz + k_0], 3, y, y2, y3,
           z, z2, z3) *
             x3;
}
...
inline Interpolator3D::Interpolator3D(const Interpolator3D& other)
    : m_nx(other.m_nx), m_ny(other.m_ny), m_nz(other.m_nz) {
  prepare_data();
  set_grid(nullptr);

  prepare_cached_coeffs();

  std::copy(other.m_data, other.m_data + (m_nx + 3) * (m_ny + 3) * (m_nz + 3),
            m_data);
  std::copy(other.m_cached_coeffs, other.m_cached_coeffs + m_nx * m_ny * m_nz,
            m_cached_coeffs);
